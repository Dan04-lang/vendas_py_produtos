...
def cadastrar_produto():
    nome = input("Digite o nome de produto: ")
    preco = float(input(f"Digite o preço de {nome}: "))
    estoque = int(input(f"Digite a quantidade de estoque de {nome}: "))
...
def exibir_produtos():
    ...
    for produto, info in produtos.items():
        print(f"Nome: {produto}, Preço: R${info['preço']:.2f}, Estoque: {info['estoque']} unidades")
    print()
...
def realizar_venda():
    ...
    if produto_vendido in produtos:
        ...
        if produtos[produto_vendido]["estoque"] >= quantidade:
            ...
            print(f"Não temos estoque suficiente de {produto_vendido} para realizar a compra")
    else:
        print(f"Produto {produto_vendido} não encontrado")
...
def sortear_promocao():
    ...
    print(f"Promoção sorteada: {produto_sorteado} com desconto de {desconto}%!\n")
